#ifndef __PERSEUS_VECTOR_3D__
#define __PERSEUS_VECTOR_3D__
...
namespace Perseus
{
	namespace Primitives
	{
		...
		public:
			...
			const Vector3D& Vector3D::operator+= (Vector3D &p){
				x += p.x; y += p.y; z += p.z;
				return *this;
			}
			...
			Vector3D Vector3D::operator* (T f) const{
				Vector3D r(x * f, y * f, z * f);
				return r;
			}

			Vector3D& Vector3D::operator*= (T f){
				x *= f; y *= f; z *= f;
				return *this;
			}
