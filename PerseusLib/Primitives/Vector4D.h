#ifndef __PERSEUS_VECTOR_4D__
#define __PERSEUS_VECTOR_4D__
...
namespace Perseus
{
	namespace Primitives
	{
		...
		public:
			...
			const Vector4D& Vector4D::operator+= (Vector4D &p){
			...
			Vector4D Vector4D::operator* (T f) const{
				Vector4D r(x * f, y * f, z * f, w * f);
				return r;
			}

			Vector4D& Vector4D::operator*= (T f){
				x *= f; y *= f; z *= f; z *= w;
				return *this;
			}
