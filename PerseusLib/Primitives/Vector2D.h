#ifndef __PERSEUS_VECTOR_2D__
#define __PERSEUS_VECTOR_2D__
...
namespace Perseus
{
	namespace Primitives
	{
		...
		public:
			...
			const Vector2D& Vector2D::operator+= (Vector2D &p){
				x += p.x; y += p.y;
				return *this;
			}
			...
			Vector2D Vector2D::operator* (T f) const{
				Vector2D r(x * f, y * f);
				return r;
			}

			Vector2D& Vector2D::operator*= (T f){
				x *= f; y *= f;
				return *this;
			}
