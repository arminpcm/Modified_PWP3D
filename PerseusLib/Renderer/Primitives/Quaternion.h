#ifndef __RENDERER_QUATERNION__
#define __RENDERER_QUATERNION__
...
namespace Renderer
{
	namespace Primitives
	{
		...
		public:			
			...
			{
				double test = this->vector4d.x*this->vector4d.y + this->vector4d.z*this->vector4d.w;
				if (test > 0.499) { // singularity at north pole
					euler->x = 2 * atan2f(this->vector4d.x,this->vector4d.w);
					euler->z = (VFLOAT) PI/2;
					euler->y = 0;
					return;
				}
				...
				double sqx = this->vector4d.x*this->vector4d.x;
				double sqy = this->vector4d.y*this->vector4d.y;
				double sqz = this->vector4d.z*this->vector4d.z;
				euler->x = atan2f((VFLOAT) (2*vector4d.y*vector4d.w-2*vector4d.x*vector4d.z ), (VFLOAT) (1 - 2*sqy - 2*sqz));
				euler->z = asinf((VFLOAT) (2*test));
				euler->y = atan2f((VFLOAT) (2*vector4d.x*vector4d.w-2*vector4d.y*vector4d.z ), (VFLOAT) (1 - 2*sqx - 2*sqz));
			}

			void ToEuler(VFLOAT *eulerx, VFLOAT *eulery, VFLOAT *eulerz) 
			{
				double test = this->vector4d.x*this->vector4d.y + this->vector4d.z*this->vector4d.w;
				if (test > 0.499) { // singularity at north pole
					*eulerx = 2 * atan2f(this->vector4d.x,this->vector4d.w);
					*eulerz = (VFLOAT) PI/2;
					*eulery = 0;
					return;
				}
				...
				double sqy = this->vector4d.y*this->vector4d.y;
				double sqz = this->vector4d.z*this->vector4d.z;
				*eulerx = atan2f((VFLOAT) (2*vector4d.y*vector4d.w-2*vector4d.x*vector4d.z ), (VFLOAT) (1 - 2*sqy - 2*sqz));
				*eulerz = asinf((VFLOAT) (2*test));
				*eulery = atan2f((VFLOAT) (2*vector4d.x*vector4d.w-2*vector4d.y*vector4d.z ), (VFLOAT) (1 - 2*sqx - 2*sqz));
			}
			...
			void FromPointAndReference(float refX, float refY, float refZ, float pointX, float pointY, float pointZ, int type)
			{
					...
					sqrtf(refX * refX + refY * refY + refZ * refZ) != 0)
				{
					...
					axisx = axisxCand[0];
					for (int i=1; i<3; i++)
					{ if (int(1000 * abs(axisx)) != int(1000 * abs(axisxCand[i]))) axisx = (abs(axisx) < abs(axisxCand[i])) ? axisx : axisxCand[i]; }
					...
					axisy = axisyCand[0];
					for (int i=1; i<3; i++)
					{ if (int(1000 * abs(axisy)) != int(1000 * abs(axisyCand[i]))) axisy = (abs(axisy) < abs(axisyCand[i])) ? axisy : axisyCand[i]; }
