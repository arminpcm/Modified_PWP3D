...
CameraCoordinateTransform::CameraCoordinateTransform(void)
{
	...
	projectionMatrixZNN = new VFLOAT[16];

	for (int i=0; i<16; i++)
	{
		projectionMatrix[i] = 0;
		projectionMatrixGL[i] = 0;
		projectionMatrixZNN[i] = 0;
	}
}
...
void CameraCoordinateTransform::SetProjectionMatrix(Camera3D* camera, VFLOAT zNear, VFLOAT zFar)
{
	...
	{
	case Camera3D::PERSEUS_CAMERA:
		{
			VFLOAT icpara[3][4];
			VFLOAT trans[3][4];
			VFLOAT p[3][3];
			VFLOAT q[4][4], qGL[4][4], qZNN[4][4];
            ...
            qZNN[0][0] = (2.0f * p[0][0] / camera->SizeX);
            qZNN[0][1] = (2.0f * p[0][1] / camera->SizeX);
            qZNN[0][2] = 0.0f;
            qZNN[0][3] = 0.0f;

            qZNN[1][0] = 0.0f;
            qZNN[1][1] = -(2.0f * p[1][1] / camera->SizeY);
            qZNN[1][2] = 0.0f;
            qZNN[1][3] = 0.0f;
			...
			{
				for (j = 0; j < 3; j++)
				{ projectionMatrixZNN[i + j * 4] = qZNN[i][0] * trans[0][j] + qZNN[i][1] * trans[1][j] + qZNN[i][2] * trans[2][j]; }
				projectionMatrixZNN[i + 3 * 4] = qZNN[i][0] * trans[0][3] + qZNN[i][1] * trans[1][3] + qZNN[i][2] * trans[2][3] + qZNN[i][3];
			}
	...
	case Camera3D::PTAM_CAMERA:
		{
			float m4[4][4];
			
			for (int i=0; i<4; i++) for (j=0; j<4; j++) m4[i][j] = 0.0f;

			float left = camera->vImplaneTL[0] * zNear;
			float right = camera->vImplaneBR[0] * zNear;
			float top = camera->vImplaneTL[1] * zNear;
			float bottom = camera->vImplaneBR[1] * zNear;

			m4[0][0] = -(2 * zNear) / (right - left);
			m4[1][1] = (2 * zNear) / (top - bottom);

			m4[0][2] = (right + left) / (left - right);
			m4[1][2] = (top + bottom) / (bottom - top);
			m4[2][2] = (zFar + zNear) / (zFar - zNear);
			m4[3][2] = -1;

			m4[2][3] = -2 * zNear * zFar / (zNear - zFar);

			projectionMatrixGL[0] = m4[0][0]; projectionMatrixGL[1] = m4[1][0];
			projectionMatrixGL[2] = m4[2][0]; projectionMatrixGL[3] = m4[3][0];
			projectionMatrixGL[4] = m4[0][1]; projectionMatrixGL[5] = m4[1][1];
			projectionMatrixGL[6] = m4[2][1]; projectionMatrixGL[7] = m4[3][1];
			projectionMatrixGL[8] = m4[0][2]; projectionMatrixGL[9] = m4[1][2];
			projectionMatrixGL[10] = m4[2][2]; projectionMatrixGL[11] = m4[3][2];
			projectionMatrixGL[12] = m4[0][3]; projectionMatrixGL[13] = m4[1][3];
			projectionMatrixGL[14] = m4[2][3]; projectionMatrixGL[15] = m4[3][3];
			...
			projectionMatrix[0] = m4[0][0]; projectionMatrix[1] = m4[1][0];
			projectionMatrix[2] = m4[2][0]; projectionMatrix[3] = m4[3][0];
			projectionMatrix[4] = m4[0][1]; projectionMatrix[5] = m4[1][1];
			projectionMatrix[6] = m4[2][1]; projectionMatrix[7] = m4[3][1];
			projectionMatrix[8] = m4[0][2]; projectionMatrix[9] = m4[1][2];
			projectionMatrix[10] = m4[2][2]; projectionMatrix[11] = m4[3][2];
			projectionMatrix[12] = m4[0][3]; projectionMatrix[13] = m4[1][3];
			projectionMatrix[14] = m4[2][3]; projectionMatrix[15] = m4[3][3];
...
void CameraCoordinateTransform::GetProjectionMatrix(VFLOAT *pmatrix, bool nornaliseZBuffer)
{
	if (nornaliseZBuffer) 
		for (int i=0; i<16; i++) pmatrix[i] = projectionMatrix[i];
	else 
		for (int i=0; i<16; i++) pmatrix[i] = projectionMatrixZNN[i];
}
