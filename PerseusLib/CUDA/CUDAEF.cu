...
__host__ void initialiseEF(int width, int height, float* heavisideFunction, int heavisideFunctionSize)
{
	...
	perseusSafeCall(cudaMalloc((void**)&cudaData->dfxTranslation, width * height * sizeof(float3))); 
	perseusSafeCall(cudaMalloc((void**)&cudaData->dfxRotation, width * height * sizeof(float4))); 

	perseusSafeCall(cudaMallocHost((void**)&cudaData->dfxResultTranslation, width * height * sizeof(float3)));
	perseusSafeCall(cudaMallocHost((void**)&cudaData->dfxResultRotation, width * height * sizeof(float4)));
}
...
__host__ void processEFD1(float* dpose, int *roiNormalised, int *roiGenerated, float2* histogram, uchar4 *imageRegistered, unsigned char *imageObjects, 
						  unsigned int *imageZBuffer, unsigned int *imageZBufferInverse, float *dt, int *dtPosX, int *dtPosY, float *dtDX, float *dtDY)
{
	size_t i;
	...
	processEFD1_global<<<blockSize, threadSize>>>(cudaData->dfxTranslation, cudaData->dfxRotation, histogram, imageRegistered, imageObjects,
		imageZBuffer, imageZBufferInverse, dt, dtPosX, dtPosY, dtDX, dtDY, roiGenerated[0], roiGenerated[1], cudaData->widthROI, cudaData->heightROI);
...
__global__ void processEFD1_global(float3 *dfxTranslation, float4 *dfxRotation, float2 *histograms, uchar4 *imageRegistered, unsigned char *imageObjects,
								   unsigned int *imageZBuffer, unsigned int *imageZBufferInverse, float *dt, int *dtPosX, int *dtPosY, float *dtDX, float *dtDY,
								   int minX, int minY, int widthROI, int heightROI)
{
	...
	if (offsetX < widthROI && offsetY < heightROI)
	{
		...
		float dtIdx = dt[offset], norm;
		float dfPosYPP = dtPosY[offset];
		...
		if (dfPosYPP >= 0)
		{
			...
			if (hidx >= 0 && hidx < 8192)
			{
				...
				histogramPixel = histograms[histOffsets[currentHistogram] + pidx];
