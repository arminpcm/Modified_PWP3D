...
{
	perseusSafeCall(cudaMalloc((void**)&cudaData->dtImageT1, width * height * sizeof(float)));
	perseusSafeCall(cudaMalloc((void**)&cudaData->dtZImage, (width+1) * height * sizeof(float)));
	perseusSafeCall(cudaMalloc((void**)&cudaData->dtVImage, width * height * sizeof(int)));
	perseusSafeCall(cudaMalloc((void**)&cudaData->dtImagePosYT1, width * height * sizeof(int)));
}
...
{
	dtToImage<<<1, cudaData->widthFull>>>(imageTransform, image, cudaData->widthFull, cudaData->heightFull);
}
...
{
	dim3 blocks(8);
	dim3 threads_in_block_t1(iDivUp(cudaData->widthROI, 8));
	dim3 threads_in_block_t2(iDivUp(cudaData->heightROI, 8));

	perseusSafeCall(cudaMemset(cudaData->dtImageT1, 0, cudaData->widthROI * cudaData->heightROI * sizeof(float)));
	perseusSafeCall(cudaMemset(dt, 0, cudaData->widthROI * cudaData->heightROI * sizeof(float)));
	perseusSafeCall(cudaMemset(dtPosX, -1, cudaData->widthROI * cudaData->heightROI * sizeof(int)));
	perseusSafeCall(cudaMemset(dtPosY, -1, cudaData->widthROI * cudaData->heightROI * sizeof(int)));

	processDTT1<<<blocks, threads_in_block_t1>>>(grayImage, cudaData->dtImageT1, dtPosX, cudaData->dtImagePosYT1, 
		cudaData->widthROI, cudaData->heightROI, cudaData->widthROI, roi[0], roi[1], roi[2], roi[3], bandSize);
	
	processDTT2<<<blocks, threads_in_block_t2>>>(dt, cudaData->widthROI, cudaData->heightROI, cudaData->dtImageT1, 
		cudaData->dtZImage, cudaData->dtVImage, signMask, dtPosX, cudaData->dtImagePosYT1, dtPosY, 
		cudaData->widthROI, cudaData->heightROI, roi[0], roi[1], roi[2], roi[3], bandSize);
}
...
{
	memset(dt, 0, cudaData->widthFull * cudaData->heightROI * sizeof(float));
