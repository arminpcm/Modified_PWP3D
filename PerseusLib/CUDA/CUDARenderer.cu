...
__host__ void initialiseRenderer(int width, int height)
{
	...
	perseusSafeCall(cudaMalloc((void**)&cudaData->depthMapObject, width * height * sizeof(unsigned char)));
	...
	perseusSafeCall(cudaMalloc((void**)&cudaData->depthMap, width * height * sizeof(unsigned int)));
	perseusSafeCall(cudaMalloc((void**)&cudaData->depthMapInverse, width * height * sizeof(unsigned int)));
...
__host__ void shutdownRenderer()
{
	...
	perseusSafeCall(cudaFree(cudaData->depthMapObject));
	...
	perseusSafeCall(cudaFree(cudaData->depthMap));
	perseusSafeCall(cudaFree(cudaData->depthMapInverse));
...
__host__ void renderObjectCUDA_SO_EF(float4 *vertices, int faceCount, int objectId, float *h_pmMatrix, int *h_viewTransform, int widthRender, int heightRender)
{
	...
	renderScene_SO_global<<<blocks, threads_in_block>>>(cudaData->fill, cudaData->objects, cudaData->zbuffer, cudaData->zbufferInverse, 
									  ...
									  int4 *rois, int faceCount, int objectId, int widthRender, int heightRender)
{
	...
	if (faceId < faceCount)
	{
		...
		norm = 1.0f / buff2.w; 
		A.x = CLAMP(viewTransformRender[0] + (float(viewTransformRender[2] / 2.0f)) * (buff2.x * norm + 1.0f), 0, widthRender - 1); 
		A.y = CLAMP(viewTransformRender[1] + (float(viewTransformRender[3] / 2.0f)) * (buff2.y * norm + 1.0f), 0, heightRender - 1); 
		...
		buff2.w = pmMatrix[3] * buff1.x + pmMatrix[7] * buff1.y + pmMatrix[11] * buff1.z + pmMatrix[15] * buff1.w;
		norm = 1.0f / buff2.w; 
		B.x = CLAMP(viewTransformRender[0] + (float(viewTransformRender[2] / 2.0f)) * (buff2.x * norm + 1.0f), 0, widthRender - 1); 
		B.y = CLAMP(viewTransformRender[1] + (float(viewTransformRender[3] / 2.0f)) * (buff2.y * norm + 1.0f), 0, heightRender - 1); 
		...
		buff2.w = pmMatrix[3] * buff1.x + pmMatrix[7] * buff1.y + pmMatrix[11] * buff1.z + pmMatrix[15] * buff1.w;
		norm = 1.0f / buff2.w; 
		C.x = CLAMP(viewTransformRender[0] + (float(viewTransformRender[2] / 2.0f)) * (buff2.x * norm + 1.0f), 0, widthRender - 1);
		C.y = CLAMP(viewTransformRender[1] + (float(viewTransformRender[3] / 2.0f)) * (buff2.y * norm + 1.0f), 0, heightRender - 1);
		...
		for(; S.y <= B.y; S.y++, E.y++, S.x += dxa, E.x += dxb, S.z += dza, E.z += dzb)
		{
			...
			{
				index = PIXELMATINDEX(i, S.y, widthRender);
				intZ = MAX_INT * Sz;

				atomicVal = atomicMin(&zbuffer[index], intZ);
				atomicMax(&zbufferInverse[index], intZ);
				if (atomicVal >= intZ) { objects[index] = objectId + 1; fill[index] = 128; }
		...
		sdataROI[offsetInBlock].x = MIN(A.x, B.x);
		sdataROI[offsetInBlock].y = MIN(A.y, B.y);
		sdataROI[offsetInBlock].z = MAX(A.x, B.x);
		sdataROI[offsetInBlock].w = MAX(A.y, B.y);

		sdataROI[offsetInBlock].x = MIN(sdataROI[offsetInBlock].x, C.x);
		sdataROI[offsetInBlock].y = MIN(sdataROI[offsetInBlock].y, C.y);
		sdataROI[offsetInBlock].z = MAX(sdataROI[offsetInBlock].z, C.x);
		sdataROI[offsetInBlock].w = MAX(sdataROI[offsetInBlock].w, C.y);
	}
